...
class sline():
    ...
    def _read_zsl(self, file_path):
        ...
        with open(file_path, 'r') as f:
            for line in f:
                ...
                lt = line.strip().split('\t')
                chr_id = lt[0]
                if chr_id == 'Chr_ID': continue
                if chr_id not in zsl:
                    zsl[chr_id] = []
                zsl[chr_id].append([int(lt[1]), int(lt[2]), abs(int(lt[1])-int(lt[2]))+1])
        return zsl
    ...
    def run(self):
        ...
        # exit()
        self.zsl = self._read_zsl("my_centri.v3-none.gff")
        chr_color = {}
        ...
        colineartlys,focus_list2,blocks = [],[],[]
        for i in range(len(lt)):
            if self.type != 'line':
                if self.focus_list == 'None':
                ...
                colineartly,block = read_coliearity_sline(lt[i],self.block,self.genepairsfile_type,chrlists[i]+chrlists[i+1])
                colineartlys.append(colineartly)
                blocks.append(block)
        # print(colineartlys)
        sumchrs = []
        ...
        total_size = 160508864 # 160508864
        step = 0.99/total_size
        GAP1 = ((total_size/0.99)*0.01)/2
        ns = []
        for lt in chrlists:
            ns.append(len(lt))
        se_dics = []
        ...
        for i in range(len(ns)):
            for chro in chrlists[i]:
                # print(chro)
                print(se_dics[i][chro]['start'],se_dics[i][chro]['end'])
                ...
                # plt.text((xmin+xmax)/2, h0-0.02,name,ha='center',va='center', fontproperties=font_prop,fontsize=int(self.chr_name_size))
                plt.text(xmin-0.01, h0+(self.h/2),name,ha='right',va='center', fontproperties=font_prop,fontsize=int(self.chr_name_size))
                if chro in self.colord.keys():
                    color = self.colord[chro]
                else:
                    color = self.colord[i]
                plt.fill(x,y, facecolor=color,alpha=1,edgecolor='black',linewidth=0.25)# self.cols.split(',')[i]

                if chro in self.zsl.keys():
                    # print(chro,self.zsl[chro],se_dics[i][chro]['start'])
                    xmin,xmax = (se_dics[i][chro]['start']+self.zsl[chro][0][0])*step,(se_dics[i][chro]['start']+self.zsl[chro][0][1])*step
                    x,y = line_chr1(h0,xmin,xmax,self.h)
                    plt.fill(x,y, facecolor='#3eb370',alpha=1)# self.cols.split(',')[i]
        ...
        for num in range(len(ns)-1):
            if self.type != 'line':
                for s1h,s2h,s1d,s2d in colineartlys[num]:
                    ...
                    if s1dpop - s1hpop > 0 and s2dpop - s2hpop > 0 or s1dpop - s1hpop < 0 and s2dpop - s2hpop < 0:
                        chro_color = '#95949a'
                    else:
                        if s1dpop - s1hpop > 0 and s1hchr[:3] == 'chr':
                            chro_color = '#ffea00'# '#e2041b'
                        else:
                            chro_color = '#ffea00'# '#e2041b'
                ...
                for n in colineartlys[num].keys():
                    data = blocks[num][n]
                    s11,s12 = data[0],data[2]
                    s11hpop,s12hpop = gff[s11][self.position],gff[s12][self.position]
                    s21,s22 = data[1],data[3]
                    s21hpop,s22hpop = gff[s21][self.position],gff[s22][self.position]
                    if s12hpop - s11hpop > 0 and s22hpop - s21hpop > 0 or s12hpop - s11hpop < 0 and s22hpop - s21hpop < 0:
                        chro_color = '#dddcd6'
                    else:
                        if s12hpop - s11hpop > 0 and s1hchr[:3] == 'chr':
                            chro_color = '#e95295'# '#ffea00'# '#e2041b'
                        else:
                            chro_color = '#e95295'# '#ffea00'# '#e2041b'

                    for s1h,s2h in colineartlys[num][n]:
                        if self.blast_reverse == "True":
                            s1h,s2h = s2h,s1h
            ...
            #                 s1hpop,s2hpop = s1hpo*step,s2hpo*step
            #                 x,y = self.drow_coli0(s1hpop,s2hpop,h0,h0-mh,True)
            #                 plt.plot(x, y, color='#95949a',alpha=self.alpha_f,linewidth=self.linewidth)
            #             elif s1hchr in chrlists[num+1] and s2hchr in chrlists[num]:# 共线性第一列就是第二排
            #                 s1hpop,s2hpop = gff[s1h][self.position],gff[s2h][self.position]
            #                 s1hchr,s2hchr = gff[s1h]['chr'],gff[s2h]['chr']
            #                 s1hpo,s2hpo = se_dics[num+1][s1hchr]['start']+s1hpop,se_dics[num][s2hchr]['start']+s2hpop
