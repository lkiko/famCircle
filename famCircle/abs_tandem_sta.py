			...
			if row[self.genome] is not np.nan:
				...
				for i in lt:
					...
					if name in gff.keys():
						...
						if name[:-1] in gff.keys():
							lt0.append(name[:-1])
						else:
							print(name,'gene id is not in gff')
							exit()
				# lt = [self.get_sta(i) for i in lt]
				lt = lt0
					...
					for x in range(len(lt)):
						for y in range(x+1,len(lt)):
							if np_matx[x][y] <= int(self.gap):
								...
								tandem.append(gff[lt[x]]['gene'])
								tandem.append(gff[lt[y]]['gene'])
