...
from famCircle.bez import drw_line, get_genedensity, get_genomeGC, rad_to_coord, read_fasta, readblast, get_path
...
class loops_ks():
	...
	def ksrun(self):
		...
		colineartly = readblast(self.genepairs,self.genepairsfile_type,self.block,self.class1)
		ks = read_ks0(self.ks)
		...
		return gff,chrlist,lens,colineartly,GC,repeatsof,repeat,ks
	...
	def draw_colineartly(self,gff,chr_list,lens,colineartly,total_size,gene_average,s_e_dic1,ks):
		for i in chr_list:
		...
		for chro in dic0:
			...
			for i in lt:
				...
				if self.chr_reverse == 'False':
					...
					if lens[chro1]['rev'] == '-':
						pos1 = lens[chro1]['end'] - pos1
					if lens[chro2]['rev'] == '-':
						pos2 = lens[chro2]['end'] - pos2

				if chro1 not in lens.keys() or chro2 not in lens.keys():
					continue
				alp = 0.05
				if str(id1) + '_' + str(id2) in ks.keys():
					...
					col = '#d4dcda'
				# self.plot_bez_inner((chro1, pos1, self.radius_a*0.95), (chro2, pos2, self.radius_a*0.95), col, total_size,alp,0.5)
		...
		for i in up:
			...
			alp = 0.5
			if str(id1) + '_' + str(id2) in ks.keys():
				ks_name = str(id1) + '_' + str(id2)
			else:
				ks_name = str(id2) + '_' + str(id1)
			ks_v = ks_v = ks[ks_name]

			col = return_col1(self.model,self.ks_concern,ks_v,float(self.ks_concern.split(',')[0]),float(self.ks_concern.split(',')[-1]))
			self.plot_bez_inner((chro1, pos1, self.radius_a*0.97), (chro2, pos2, self.radius_a*0.97), col, total_size,alp,0.1)
		...
		# 	x,y = plot_colineartly_block(self.start_list,[chroa,chrob],[pos1,pos2,pos3,pos4], self.radius_a*0.99,total_size)
	...
	def read_data(self,soft,pvuv,x_index,y_index,chr_list,s_e_dic,gff,chr_dic,total_size,r_min,r_max,col,lens,font_prop):
		...
		for key in dic.keys():
			...
			if key == chr_list[0]:
				...
				text(label_x, label_y, soft, ha='left',va='center', horizontalalignment="center", verticalalignment="center", color = 'black',rotation=-90, fontproperties=font_prop, fontsize = self.softfont)
			else:
				pass
			x1 = []# 低于均值
			y1 = []
	...
	def draw_loops(self,gff,chr_list,lens,GC,repeatsof,repeat,loopN,s_e_dic,total_size,font_prop):
		cols = ['#2ca9e1','#007bbb','#eb6238','#68be8d','#745399','#867ba9','#b3ada0','#028760','#ee836f','#e7609e']
		if self.genomefile != 'None':
			# print('GC')
	...
	def run(self):
		# print("2023.8.8修订！")
		gff,chr_list,lens,colineartly,GC,repeatsof,repeat,ks = self.ksrun()
		loopN = 0
		if len(GC) != 0:
			loopN += 1
		if len(repeatsof) != 0:
		...
		self.draw_colineartly(gff,chr_list,lens,colineartly,total_size,gene_average,s_e_dic1,ks)
		...
		if self.model == 'gradual':
			...
			drawSpec_1(self.ks_concern,gs[8:22,27:],self.makerfont,font_prop)
