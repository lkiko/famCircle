...
class outer():
    ...
    def run(self):
                ...
                if id1 not in family or id2 not in family:
                    ...
                    if chro1==chro2:
                        ...
                        if ks_v < float(self.ks_concern.split(',')[0]) or ks_v > float(self.ks_concern.split(',')[1]):
                            ...
                            lt01_o.append([id1,id2])# 染色体之间
                            continue
                        # self.plot_bez_inner((chro1, pos1, self.radius_a*0.95), (chro2, pos2, self.radius_a*0.95), "#d4dcda", total_size,0.2)#绘制内部共线性
            else:
                self.plot_bez_inner((chro1, pos1, self.radius_a*0.95), (chro2, pos2, self.radius_a*0.95), "#d4dcda", total_size,0.2)#绘制内部共线性

        output = open("outer_"+str(self.ks_concern.split(',')[0])+"_"+str(self.ks_concern.split(',')[1])+".outer","w")
        output.write("id1\tid2\tchro1\tchro2\tstart1\tstart2\tks_v\tdistance\tTYPE\n")

        # print("染色体间绘制")
        if self.parameter == 'True':
            for j in trange(len(lt01_o), desc='Between chromosomes ->'):
                i = lt01_o[j]
                id1,id2 = i[0],i[1]
                ks_v = ks[str(id1) + '%' + str(id2)]
                pos1,pos2 = gff[id1]['end'] + (total_size*self.block_scize/2),gff[id2]['end'] + (total_size*self.block_scize/2)
                order1,order2 = gff[id1]['order'],gff[id2]['order']
                chro1,chro2 = gff[id1]['chr'],gff[id2]['chr']
                ...
                +"\t"+str(ks_v)+"\tNULL\tbetween_chromosomes\n")
        ...
        for i in dic_motif.keys():
            ...
            plt.text(0.31, y-0.002, str(i)+"_"+str(dic_motif[i]), fontproperties=font_prop,fontsize=self.makerfont)
            y += 0.015

        ratio = 0.5
        # print("绘制染色体内部")
        # print(fam_clu)
        for j in trange(len(lt01_i), desc='Inside chromosomes ->'):
            key = lt01_i[j]
            id1,id2 = key[0],key[1]

            if id1 in family and id2 in family and self.cluster0(cluster,id1,id2):# 基因簇内部
                ks_v = ks[str(id1) + "%" + str(id2)]
                ...
                if ks_v >= float(self.ks_concern.split(',')[0]) and ks_v <= float(self.ks_concern.split(',')[1]):
                    ...
                    # self.plot_bez_inner(start1, self.radius_a*0.98, start2, self.radius_a*0.98 , col, ratio)
                    self.plot_bez_inner((chro1, pos1, self.radius_a*0.98), (chro2, pos2, self.radius_a*0.98), col, total_size,1)#绘制内部共线性
        output.close()
        plt.axis('off')
