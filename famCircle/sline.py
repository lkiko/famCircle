...
class sline():
    ...
    def run(self):
        ...
        total_size = ((self.gap+1)*max(sumchrs)/self.gap)
        ...
        name = ['NoclMET1','NsaMP1776','NgaMP1894']
        for i in range(len(ns)):
            plt.text(-0.06, h0+0.01,name[i],ha='center',va='center', fontproperties=font_prop,fontsize=12)
            for chro in chrlists[i]:
                # print(chro)
                xmin,xmax = se_dics[i][chro]['start']*step,se_dics[i][chro]['end']*step
                x,y = line_chr(h0,xmin,xmax,self.h)
                if chro[:2] == 'CM' or chro[:2] == 'SD':
                    pass
                else:
                    plt.text((xmin+xmax)/2, h0+0.01,chro,ha='center',va='center', fontproperties=font_prop,fontsize=int(self.chr_name_size))
                plt.fill(x,y, facecolor=self.cols.split(',')[i],alpha=.7)
            h0 -= mh
                ...
                if s1hchr != s2hchr:# 如果当前block在不同染色体上
                    ...
                    if self.genepairsfile_type == 'MCScanX':
                        if s1hchr in chrlists[num] and s2dchr in chrlists[num+1]:# 共线性第一列就是第二排
                            ...
                            plt.fill(x, y, facecolor=chro_color,alpha=.7)
